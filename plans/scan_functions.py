...
from config.runengine_Copy1 import LiveStatsPlot
from utils.logger import append_metadata_to_csv
from utils.eiger_funcs import configure_eiger_for_burst
from time import strftime
# from tiled.catalog.utils import write_single_run
...
def run_burst_scan(
    ...
    effective_frame_rate = nframes / (frame_time * nframes + 0.1)
    ...
    run = RE(_scan(), md)
    scan_num = RE.md['scan_id']

    # Export to HDF5 if supported
    try:
        latest = catalog[-1]
        h5name = f"{file_prefix}_{sample_name}_{scan_num}_{timestamp}.h5"
    ...
    # Also write to Tiled catalog if using native Tiled
    try:
        write_single_run(run, catalog, metadata=md)
        print("📤 Run inserted into Tiled native catalog.")
    except Exception as e:
        print(f"⚠️ Tiled native insert failed: {e}")
